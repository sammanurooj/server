...
class UserController {
  ...
  static getRouter() {
    ...
    this.router.get('/Card', this.list);
  ...
  static async list(req, res, next) {
    try {
      const users = await marketingCard.findAndCountAll({});
  ...
  static async getCardById(req, res, next) {
    ...
    try {
      ...
      const user = await marketingCard.findOne({ where: { id } });
